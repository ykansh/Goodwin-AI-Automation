    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm overflow-y-auto">
      <div className="w-full max-w-lg glass-strong rounded-3xl shadow-2xl border border-white/70 overflow-hidden my-8 animate-scale-in">
        <div className="p-6 bg-gradient-to-r from-[#00a631] to-[#008a29] text-white flex items-center justify-between">
          <div>
            <h2 className="text-xl font-extrabold tracking-wide flex items-center gap-2">
              <ArrowDownLeft className="w-5 h-5 text-[#cde06c]" /> Take Payment In
            </h2>
            <p className="text-xs text-emerald-100 mt-0.5">
              Customer outstanding will decrease & Cash/Bank balance will increase
            </p>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <label className="block text-xs font-bold text-[#3a3b39] mb-1">Select Customer / Party</label>
            <select
              value={customerId}
            ...
            >
              {customers.map((c) => (
                <option key={c.id} value={c.id}>
                  {c.name} ({c.uoi}) - Current Outstanding: ₹{c.outstanding.toLocaleString('en-IN')}
                </option>
              ))}
            </select>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-bold text-[#3a3b39] mb-1">Receipt Date</label>
              <input
                type="date"
                value={date}
          ...
          <div>
            <label className="block text-xs font-bold text-[#3a3b39] mb-1">Reference # / UTR / Cheque #</label>
            <input
              type="text"
              required
              value={reference}
          ...
          <div>
            <label className="block text-xs font-bold text-[#3a3b39] mb-1">Amount Received (₹)</label>
            <input
              type="number"
              min="1"
              required
              ...
              placeholder="0"
              className="w-full glass-input px-3 py-2.5 text-base font-extrabold text-[#00a631]"
            />
          </div>
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            >
              <CheckCircle className="w-4 h-4" /> Save Receipt & Post Ledger
            </button>
          </div>
        </form>
      </div>
